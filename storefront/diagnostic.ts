...
async function check() {
  const { data } = await supabase.from('productos').select('genero, activo, categoria')
  const genders = [...new Set(data?.map(i => i.genero))]
  console.log('Unique Genders in DB:', genders)
}
check()
